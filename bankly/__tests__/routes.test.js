...
describe("GET /users", function() {
  ...
  // TEST BUG #1 //
  test("should not get any sensitive information", async function() {
    const response = await request(app)
      .get("/users")
      .set("Authorization", `Bearer ${tokens.u1}`);
  ...
  // TEST BUG #6 //
  test("should deny access if incorrect token provided", async function() {
    let payload = {username: "fakeUser", admin: "True"};
    let uFake = jwt.sign(payload, "differentKey");
    const response = await request(app)
      .get("/users")
      .set("Authorization", `Bearer ${uFake}`);
      expect(response.statusCode).toBe(401);
  });
});
...
// TESTS BUG #2 //
  test("should return 404 if user doesn't exist", async function() {
    const response = await request(app).get("/user/u5");
...
describe("PATCH /users/[username]", function() {
  ...
  test("allow patch to user's own profile, even if not admin", async () => {
    const response = await request(app)
    .patch("/users/u1")
    .set("Authorization", `Bearer ${tokens.u1}`) // u2 is not admin // TESTS BUG #3 //
